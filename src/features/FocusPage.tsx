...
export function FocusPage(){
  ...
  async function completeLinked(){if(scriptId&&workflowStage){const field=workflowStage==='Registrazione'?'recorded':workflowStage==='Montaggio'?'edited':workflowStage==='Programmazione'?'scheduled':'published';await db.scripts.update(scriptId,{[field]:true,updatedAt:now()});queueSync()}if(taskId){await db.tasks.update(taskId,{completed:true,updatedAt:now()});queueSync()}setFinished(false)}
