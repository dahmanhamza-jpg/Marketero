...
export function FocusPage(){
  ...
  const [minutes,setMinutes]=useState(25);const taskId=params.get('task')||'';const [running,setRunning]=useState(false);const [paused,setPaused]=useState(false);const [remaining,setRemaining]=useState(25*60);const [startedAt,setStartedAt]=useState('');const [earned,setEarned]=useState(0);const [finished,setFinished]=useState(false);
  ...
  return <main className='page focus-page'><PageHeader eyebrow='TEMPO PROTETTO' title='Entra nel flusso' description='Scegli il tempo. Il resto può aspettare.'/><div className='focus-layout'><Card className='focus-launch premium-surface'><div className='focus-launch-top'><div><span className='eyebrow'>SESSIONE</span><h2>{minutes} minuti</h2></div><TimerReset/></div><div className='chips focus-duration'>{durations.map(m=><Chip key={m} active={minutes===m} onClick={()=>setMinutes(m)}>{m}</Chip>)}</div><PrimaryButton className='focus-start' onClick={start}>Inizia Focus</PrimaryButton></Card><Card className='level-card'><div className='row spread'><div><span className='eyebrow'>MOMENTUM</span><h2>Progressi · {level.name}</h2></div><ProgressRing value={Math.round(level.current/level.needed*100)} size={84}/></div><p><b>{total}</b> punti totali · {level.remaining} al prossimo livello</p></Card><div className='focus-stats'><Stat value={today+' min'} label='Oggi'/><Stat value={Math.floor(week/60)+'h '+week%60+'m'} label='Ultimi 7 giorni'/></div></div>{earned>0&&<RewardToast points={earned}/>} {finished&&<Card className='focus-finished'><span className='eyebrow'>SESSIONE SALVATA</span><h2>Focus completato</h2><p>{earned?('+'+earned+' Momentum · '):''}sessione registrata.</p>{(taskId||scriptId)&&<div className='row'><PrimaryButton onClick={completeLinked}>Segna attività completata</PrimaryButton><button className='btn soft' onClick={()=>setFinished(false)}>Non ancora</button></div>}</Card>}</main>;
}
