...
function PaymentsPage(){
  const payments=useLiveQuery(()=>db.payments.toArray(),[])||[]; const clients=useLiveQuery(()=>db.clients.toArray(),[])||[]; const [range,setRange]=useState(6); const [newPayment,setNewPayment]=useState(false);
  const active=payments.filter(p=>!p.deletedAt); const paid=active.filter(p=>p.paidAt); const pending=active.filter(p=>!p.paidAt); const total=paid.reduce((s,p)=>s+p.amount,0);
  const months=Array.from({length:range},(_,i)=>{const d=new Date();d.setMonth(d.getMonth()-(range-1-i));return d});
  const series=months.map(d=>{const amount=paid.filter(p=>{const x=new Date(p.paidAt!);return x.getMonth()===d.getMonth()&&x.getFullYear()===d.getFullYear()}).reduce((s,p)=>s+p.amount,0);const prev=new Date(d);prev.setMonth(prev.getMonth()-range);const prevAmount=paid.filter(p=>{const x=new Date(p.paidAt!);return x.getMonth()===prev.getMonth()&&x.getFullYear()===prev.getFullYear()}).reduce((s,p)=>s+p.amount,0);return {label:new Intl.DateTimeFormat('it-IT',{month:'short'}).format(d),amount,prevAmount}}); const max=Math.max(1,...series.flatMap(x=>[x.amount,x.prevAmount]));
  async function togglePaid(p:Payment){await db.payments.update(p.id,{paidAt:p.paidAt?undefined:now(),updatedAt:now()});queueSync();}
  return <main className="page"><header className="page-title"><div><h1>Entrate</h1><p>Incassato, da ricevere e andamento nel tempo.</p></div><PrimaryButton onClick={()=>setNewPayment(true)}>+ Pagamento</PrimaryButton></header><div className="bento"><Card><span className="eyebrow">INCASSATO</span><h2>€ {total.toLocaleString('it-IT')}</h2></Card><Card><span className="eyebrow">DA RICEVERE</span><h2>€ {pending.reduce((s,p)=>s+p.amount,0).toLocaleString('it-IT')}</h2></Card><Card><span className="eyebrow">SCADUTI</span><h2>{pending.filter(p=>paymentStatus(p)==='Scaduto').length}</h2></Card></div><Card className="chart-card"><div className="section-head"><div><h2>Andamento entrate</h2><p>Periodo precedente mostrato in trasparenza.</p></div><div className="chips">{[1,3,6,12].map(n=><Chip key={n} active={range===n} onClick={()=>setRange(n)}>{n===1?'1 mese':n===12?'1 anno':`${n} mesi`}</Chip>)}</div></div><div className="revenue-chart">{series.map((x,i)=><div className="bar-group" key={i}><div className="bars"><span className="bar previous" style={{height:`${(x.prevAmount/max)*100}%`}}/><span className="bar current" style={{height:`${(x.amount/max)*100}%`}}/></div><small>{x.label}</small></div>)}</div></Card><div className="list payment-list">{active.slice().sort((a,b)=>b.dueDate.localeCompare(a.dueDate)).map(p=>{const c=clients.find(x=>x.id===p.clientId);return <Card key={p.id}><div className="row spread"><div><h3>€ {p.amount.toLocaleString('it-IT')} · {c?.name||'Cliente'}</h3><p>Scadenza {fmtDate(p.dueDate)} · {paymentStatus(p)}</p></div><button className={`btn ${p.paidAt?'ghost':'primary'}`} onClick={()=>togglePaid(p)}>{p.paidAt?'Annulla incasso':'Segna incassato'}</button></div></Card>})}</div>{newPayment&&<PaymentForm close={()=>setNewPayment(false)}/>}</main>
}
...
function LockGate(){
  const settings=useLiveQuery(()=>db.settings.get('settings'),[]); const [unlocked,setUnlocked]=useState(false); const [mode,setMode]=useState<'unlock'|'setup'|'confirm'|'recovery'|'reset'>('unlock'); const [pin,setPin]=useState(''); const [firstPin,setFirstPin]=useState(''); const [msg,setMsg]=useState('');
  useEffect(()=>{ensureSettings().then(s=>setMode(s.pinHash?'unlock':'setup'))},[]);
  useEffect(()=>{if(pin.length!==4)return;const run=async()=>{setMsg('');if(mode==='unlock'){if(await verifyPin(pin,settings?.pinHash)){setUnlocked(true)}else{setMsg('PIN non corretto');setPin('')}}else if(mode==='recovery'){if(await verifyRecoveryCode(pin)){setPin('');setMode('reset');setMsg('Codice accettato. Imposta un nuovo PIN.')}else{setMsg('Codice di recupero non valido');setPin('')}}else if(mode==='setup'||mode==='reset'){setFirstPin(pin);setPin('');setMode('confirm')}else if(mode==='confirm'){if(pin!==firstPin){setMsg('I PIN non coincidono. Riprova.');setPin('');setMode(settings?.pinHash?'reset':'setup');return}const hash=await makePinHash(pin);await db.settings.update('settings',{pinHash:hash,firstRunDone:true,updatedAt:now()});await syncRemote().catch(()=>{});setUnlocked(true)}};run()},[pin,mode,settings?.pinHash,firstPin]);
