...
function LockGate(){
  ...
  useEffect(()=>{if(pin.length!==4)return;const run=async()=>{setMsg('');if(mode==='unlock'){if(await verifyPin(pin,settings?.pinHash)){setCinematic(true);window.setTimeout(()=>{setUnlocked(true);setCinematic(false)},980)}else{setMsg('PIN non corretto');setPin('')}}else if(mode==='recovery'){if(await verifyRecoveryCode(pin)){setPin('');setMode('reset');setMsg('Codice accettato. Imposta un nuovo PIN.')}else{setMsg('Codice di recupero non valido');setPin('')}}else if(mode==='setup'||mode==='reset'){setFirstPin(pin);setPin('');setMode('confirm')}else if(mode==='confirm'){if(pin!==firstPin){setMsg('I PIN non coincidono. Riprova.');setPin('');setMode(settings?.pinHash?'reset':'setup');return}const hash=await makePinHash(pin);await db.settings.update('settings',{pinHash:hash,firstRunDone:true,updatedAt:now()});await syncRemote().catch(()=>{});setCinematic(true);window.setTimeout(()=>{setUnlocked(true);setCinematic(false)},980)}};run()},[pin,mode,settings?.pinHash,firstPin]);
  if(unlocked||cinematic)return <div className={`unlock-stage ${cinematic?'is-unlocking':'is-ready'}`}>
    <div className="unlock-home-layer"><AppShell/></div>
    {cinematic&&<div className="unlock-cinematic" aria-label="Accesso in corso">
      <div className="unlock-glow"/>
      <div className="cinematic-core"><div className="cinematic-logo">M</div><span className="cinematic-line"/><div className="cinematic-orbit"/></div>
    </div>}
  </div>;
