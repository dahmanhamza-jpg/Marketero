...
function Home(){
  ...
  return <main className="page home"><header className="top"><div><h1>{greeting()} 👋</h1><p>{new Intl.DateTimeFormat('it-IT',{weekday:'long',day:'numeric',month:'long'}).format(new Date())}</p></div></header>
    ...
    <div className="week-pulse"><div className="week-pulse-head"><span className="eyebrow">LA TUA SETTIMANA</span><small>Più scuro = più impegni</small></div><div className="day-strip">{Array.from({length:5},(_,i)=>{const d=new Date();d.setDate(d.getDate()+i);const count=events.filter(e=>!e.deletedAt&&sameDay(e.startAt,d)).length+tasks.filter(t=>!t.deletedAt&&t.dueAt&&sameDay(t.dueAt,d)&&!t.completed).length;const level=count===0?0:count<=2?1:count<=4?2:count<=6?3:4;const selected=selectedDay?sameDay(selectedDay,d):i===0;return <button className={'heat-'+level+(selected?' selected':'')} key={i} onClick={()=>setSelectedDay(new Date(d))}><span>{new Intl.DateTimeFormat('it-IT',{weekday:'short'}).format(d)}</span><strong>{d.getDate()}</strong><small>{count||'libero'}</small></button>})}</div></div>
