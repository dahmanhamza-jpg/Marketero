...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  try{
    ...
    if(![1,2].includes(body?.schemaVersion)||!body.data) return Response.json({error:'Payload di sincronizzazione non valido'},{status:400});
    ...
    return Response.json({schemaVersion:2,exportDate:new Date().toISOString(),data});
